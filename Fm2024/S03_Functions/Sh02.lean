...
Lean's inbuilt types like `ℕ` and `ℝ` you might just want to
let `X = {a, b}` and `Y = {c}` and define `f(a) = f(b) = c`.
In this sheet we learn how to do this.
...
example : a ≠ b :=
  ...
  intro h
  -- `h : a = b`
  -- There are no cases => no need to prove anything.
  cases h
